...
extern dodge_section;
u32 menu_phase = 0;
s32 current_turn; // 0 mario // 1 bowser // 2 luigi // 3 check // 4 attack // 5 dodge
u32 mario_enemy_selected; // 1 enemy 1, 2 enemy 2, 3 mario, 4 bowser, 5 luigi
u32 luigi_enemy_selected;
u32 bowser_enemy_selected;

u32 spawn_one_battlebox;
extern vertical_option_max;
u32 mario_action_type; // 1 fight // 2 act // 3 heal // 4 spare // 5 defend
u32 luigi_action_type;
u32 bowser_action_type;
extern enemy_selected;
u32 mario_action_selected;
u32 luigi_action_selected;
u32 bowser_action_selected;
extern CurrTP;
extern MarioOWX;
extern MarioOWY;
extern MarioOWZ;
...
extern enemy_2_max_health;
u32 enemy_1_mercy;
u32 enemy_2_mercy;
u32 rpg_init_timer;
u32 enemy_1_dead;
u32 enemy_2_dead;
u32 enemy_1_init_explosion;
u32 enemy_2_init_explosion;
u32 explosion_timer1;
u32 explosion_timer2;
u32 Battle_Won;
extern checked;
...
if(gMarioState->floor->type != SURFACE_DIALOG){
    ...
    play_once = 1;
        ...
        if(RPG_mode == 1){
            ...
            if ((enemy_2_health <= 0) && enemy_2_dead == 0){
                enemy_2_init_explosion = 1;
                play_sound(SOUND_GENERAL_BREAK_BOX, gGlobalSoundSource);
                frame13 = 0;
                enemy_2_dead = 1;
            }
}
            if (killed >= 14 && RPG_mode == 0){
                if (gCurrLevelNum == LEVEL_WF){
    set_background_music(0, SEQ_STREAMED_DARKCITY, 0);
                }
}

        
//print_text_fmt_int(20,20, "enemy_1_init_explosion: %d", enemy_1_init_explosion);
        if (rpg_initializer >= 1){
                if (rpg_init_timer == 1){
                    MarioOWX = gMarioState->pos[0];
                MarioOWY = gMarioState->pos[1];
                MarioOWZ = gMarioState->pos[2];
                    if (killed < 13){
                fadeout_background_music(SEQ_STREAMED_CITY, 300);
                    }
                    if (killed >= 14){
                fadeout_background_music(SEQ_STREAMED_DARKCITY, 300);
                    }
            ...
            enemy_1_dead = 0;
            enemy_2_dead = 0;
            enemy_1_init_explosion = 0;
            enemy_2_init_explosion = 0;
            enemy_1_mercy = 0;
            enemy_2_mercy = 0;
            CurrTP = 0;
            framedelay = 0;
                ...
                play_transition(WARP_TRANSITION_FADE_FROM_COLOR, 15, 0, 0, 0);
        ...
        if (rpg_denitializer >= 1){
            ...
            framedelay = 0;
                ...
                if (killed >= 14){
                play_music(SEQ_PLAYER_LEVEL, SEQUENCE_ARGS(4, SEQ_STREAMED_DARKCITY), 0);
                }
                reset_camera(gCurrentArea->camera);
                rpg_denitializer = 0;
                RPG_mode = 0;
...
framedelay++;
    ...
    if ((((gPlayer1Controller->stickX < -0.5f) && framedelay > 6) && vertical_menu == 0)  && dodge_section == 0) {
        ...
        if (((gPlayer1Controller->buttonPressed & A_BUTTON) && framedelay > 6) && dodge_section == 0){
            if (menu_phase == 0){
                if (battle_option == 0) {
                vertical_menu = 1;
                vertical_option = 1;
                ...
                vertical_menu = 1;
                vertical_option = 1;
                menu_phase = 2; // phase 2 item select
                framedelay = 0;
                vertical_option_max = 1;
                play_sound(SOUND_GENERAL_LEVEL_SELECT_CHANGE, gGlobalSoundSource);
                    }
            ...
            if ((menu_phase == 3) && framedelay > 6){ // phase 3 party member select
                    if (current_turn == 0){
                        if (vertical_option == 1){
                        mario_enemy_selected = 3;
                        mario_action_type = 3;
                        }
                        if (vertical_option == 2){
                        mario_enemy_selected = 4;
                        mario_action_type = 3;
                        }
                        if (vertical_option == 3){
                        mario_enemy_selected = 5;
                        mario_action_type = 3;
                        }       
                    }
                    if (current_turn == 1){
                        if (vertical_option == 1){
                        bowser_enemy_selected = 3;
                        bowser_action_type = 3;
                        }
                        if (vertical_option == 2){
                        bowser_enemy_selected = 4;
                        bowser_action_type = 3;
                        }
                        if (vertical_option == 3){
                        bowser_enemy_selected = 5;
                        bowser_action_type = 3;
                        }       
                    }
                    if (current_turn == 2){
                        if (vertical_option == 1){
                        luigi_enemy_selected = 3;
                        luigi_action_type = 3;
                        }
                        if (vertical_option == 2){
                        luigi_enemy_selected = 4;
                        luigi_action_type = 3;
                        }
                        if (vertical_option == 3){
                        luigi_enemy_selected = 5;
                        luigi_action_type = 3;
                        }       
                    }
                    current_turn++;
                ...
                if ((menu_phase == 4) && framedelay > 6){ // phase 4 act select
                    if (current_turn == 0){
                        vertical_option_max = 3;
                        if (vertical_option == 1){
                        mario_action_selected = 1;
                        mario_action_type = 2;
                        current_turn++;
                ...
                if ((battle_option == 4)){ //menu phase 1 enemy select
                if (current_turn == 0){
        ...
        if ((gPlayer1Controller->buttonPressed & B_BUTTON) && dodge_section == 0){
            if (vertical_menu != 0){
                vertical_menu = 0;
                vertical_option = 1;
                menu_phase = 0;
        ...
        if ((((gPlayer1Controller->stickY < -0.5f) && framedelay > 6) && vertical_menu == 1) && dodge_section == 0) {
        ...
        if ((((gPlayer1Controller->buttonPressed & A_BUTTON) && framedelay > 6) && vertical_menu == 1) && dodge_section == 0) {
            if (battle_option == 0){
                framedelay = 0;
                if (current_turn == 0){
                    if((vertical_option == 1) && enemy_1_health > 0){
                            mario_enemy_selected = 1;
                            mario_action_type = 1;
                            enemy_selected = 1;
                            menu_phase = 0;
                            current_turn++;
                    ...
                    if((vertical_option == 2) && enemy_2_health > 0){
                            mario_enemy_selected = 2;
                            mario_action_type = 1;
                            enemy_selected = 2;
                            menu_phase = 0;
                            current_turn++;
            ...
            if ((battle_option == 1) && menu_phase == 1){ //menu phase 4 ACT select
                if (current_turn == 0){
                    vertical_option_max = 3;
                    if((vertical_option == 1) && enemy_1_health > 0){
                            framedelay = 0;
            ...
            if (battle_option == 2){
                if (menu_phase == 2){
                    menu_phase = 3; //menu phase 3, selecting party members
                    vertical_option_max = 3;
                    play_sound(SOUND_GENERAL_LEVEL_SELECT_CHANGE, gGlobalSoundSource);
                }
            }
        if ((battle_option == 3) && menu_phase == 1){ //menu phase 1 enemy select
                if (current_turn == 0){
                    if((vertical_option == 1) && enemy_1_health > 0){
                            mario_enemy_selected = 1;
                            mario_action_type = 4;
        ...
        //                                  bhvStaticObject, -1, 3223, -376, 0, 180, 0);
        //spawn_one_battlebox = 1;
        //}
        }
        reset_camera(gCurrentArea->camera);
...
void mode_8_directions_camera(struct Camera *c) {
    ...
    if (gPlayer1Controller->buttonPressed & R_CBUTTONS && gMarioState->floor->type != SURFACE_DIALOG) {
        s8DirModeYawOffset += DEGREES(45);
        play_sound_cbutton_side();
    }
    if (gPlayer1Controller->buttonPressed & L_CBUTTONS && gMarioState->floor->type != SURFACE_DIALOG) {
        s8DirModeYawOffset -= DEGREES(45);
        play_sound_cbutton_side();
    }
...
void update_camera(struct Camera *c) {
    ...
    if (c->cutscene == CUTSCENE_NONE
        ...
        if (cam_select_alt_mode(CAM_SELECTION_NONE) == CAM_SELECTION_MARIO) {
            if (gPlayer1Controller->buttonPressed & R_TRIG && gMarioState->floor->type != SURFACE_DIALOG) {
