...
#include "actors/Mario_Health/header.h"
#include "actors/Bowser_Health/header.h"
#include "actors/Luigi_Health/header.h"
#include "actors/Battle_Actions/header.h"
#include "actors/TP_Meter/header.h"
#include "actors/TP_Mask/header.h"
#include "actors/TP_Red/header.h"
#include "actors/Target/header.h"
#include "actors/Enemy1_Bars/header.h"
#include "actors/Enemy2_Bars/header.h"
...
u32 MarioHealthMove;
u32 LuigiHealthMove;
u32 BowserHealthMove;
u32 Enemy1HealthMove;
u32 Enemy2HealthMove;
u32 Enemy1MercyMove;
u32 Enemy2MercyMove;
u32 TPMeterMove;
u32 MarioMinPos = 64;
u32 LuigiMinPos = 279;
u32 BowserMinPos = 172;
u32 MarioMaxPos = 103;
u32 LuigiMaxPos = 317;
u32 BowserMaxPos = 210;
s32 menu_pos = 0;
extern MarioMaxHp;
extern LuigiMaxHp;
extern BowserMaxHp;
...
u32 HealthRatioMario = 39;
u32 HealthRatioBowser = 38;
u32 HealthRatioLuigi = 38;
u32 battle_pos;
u32 TPMinPos = 124;
u32 TPRatio = 94;
u32 Enemy1MinHealthPos = 221;
u32 Enemy1MinMercyPos = 280;
u32 Enemy2MinHealthPos = 221;
u32 Enemy2MinMercyPos = 280;
u32 Enemy1Ratio = 38;
u32 Enemy2Ratio = 38;
u32 Enemy1MercyRatio;
u32 Enemy2MercyRatio;
extern enemy_1_health;
extern enemy_2_health;
extern enemy_1_spare;
extern enemy_2_spare;
...
create_dl_translation_matrix(G_MTX_PUSH, 0, menu_pos, 0);
gSPDisplayList(gDisplayListHead++, &Battle_Battle_mesh);
gSPPopMatrix(gDisplayListHead++, G_MTX_MODELVIEW);

create_dl_translation_matrix(G_MTX_PUSH, 0, menu_pos, 0);
gSPDisplayList(gDisplayListHead++, &Mario_Health_Cube_001_mesh);
gSPPopMatrix(gDisplayListHead++, G_MTX_MODELVIEW);

create_dl_translation_matrix(G_MTX_PUSH, 0, menu_pos, 0);
gSPDisplayList(gDisplayListHead++, &Luigi_Health_Cube_003_mesh);
gSPPopMatrix(gDisplayListHead++, G_MTX_MODELVIEW);

create_dl_translation_matrix(G_MTX_PUSH, 0, menu_pos, 0);
gSPDisplayList(gDisplayListHead++, &Bowser_Health_Cube_002_mesh);
gSPPopMatrix(gDisplayListHead++, G_MTX_MODELVIEW);

create_dl_translation_matrix(G_MTX_PUSH, battle_pos, menu_pos, 0);
gSPDisplayList(gDisplayListHead++, &Battle_Actions_Battle_Actions_mesh);
gSPPopMatrix(gDisplayListHead++, G_MTX_MODELVIEW);

create_dl_translation_matrix(G_MTX_PUSH, 0, 0, 0);
gSPDisplayList(gDisplayListHead++, &TP_Red_TP_Red_mesh);
gSPPopMatrix(gDisplayListHead++, G_MTX_MODELVIEW);

create_dl_translation_matrix(G_MTX_PUSH, 0, 0, 0);
gSPDisplayList(gDisplayListHead++, &TP_Mask_TP_Mask_mesh);
gSPPopMatrix(gDisplayListHead++, G_MTX_MODELVIEW);
...
//create_dl_translation_matrix(G_MTX_PUSH, 214, 0, 0);
//gSPDisplayList(gDisplayListHead++, &Battle_Actions_Cube_001_mesh);
//gSPPopMatrix(gDisplayListHead++, G_MTX_MODELVIEW);

}
//create_dl_translation_matrix(G_MTX_PUSH, 52, Cursor, 50);
//gSPDisplayList(gDisplayListHead++, &MenuPointer_MenuPointer_mesh);
//gSPPopMatrix(gDisplayListHead++, G_MTX_MODELVIEW);
//}
...
if (menu_phase == 1){
    if (enemy_1_health > 0){
    create_dl_translation_matrix(G_MTX_PUSH, 0, 0, 0);
    gSPDisplayList(gDisplayListHead++, &Enemy1_Bars_Enemy__Bars_mesh);
    gSPPopMatrix(gDisplayListHead++, G_MTX_MODELVIEW);

    Vtx *Enemy1HealthMove = segmented_to_virtual(&Enemy1_Bars_Enemy__Bars_mesh_vtx_1);
    Enemy1HealthMove[9].v.ob[0] = Enemy1MinHealthPos + Enemy1Ratio;
    Enemy1HealthMove[10].v.ob[0] = Enemy1MinHealthPos + Enemy1Ratio;

    Vtx *Enemy1MercyMove = segmented_to_virtual(&Enemy1_Bars_Enemy__Bars_mesh_vtx_1);
    ...
    Vtx *Enemy2HealthMove = segmented_to_virtual(&Enemy2_Bars_Enemy__Bars_mesh_vtx_1);
    Enemy2HealthMove[9].v.ob[0] = Enemy2MinHealthPos + Enemy2Ratio;
    Enemy2HealthMove[10].v.ob[0] = Enemy2MinHealthPos + Enemy2Ratio;

    Vtx *Enemy2MercyMove = segmented_to_virtual(&Enemy2_Bars_Enemy__Bars_mesh_vtx_1);
    Enemy2MercyMove[12].v.ob[0] = Enemy2MinMercyPos + enemy_2_spare;
    Enemy2MercyMove[15].v.ob[0] = Enemy2MinMercyPos + enemy_2_spare;
    }
}



Vtx *MarioHealthMove = segmented_to_virtual(&Mario_Health_Cube_001_mesh_vtx_0);
    MarioHealthMove[0,1].v.ob[0] = MarioMinPos + HealthRatioMario;
    MarioHealthMove[0,2].v.ob[0] = MarioMinPos + HealthRatioMario;

Vtx *LuigiHealthMove = segmented_to_virtual(&Luigi_Health_Cube_003_mesh_vtx_0);
     LuigiHealthMove[0,1].v.ob[0] = LuigiMinPos + HealthRatioLuigi;
    LuigiHealthMove[0,2].v.ob[0] = LuigiMinPos + HealthRatioLuigi;

Vtx *BowserHealthMove = segmented_to_virtual(&Bowser_Health_Cube_002_mesh_vtx_0);
    BowserHealthMove[0,1].v.ob[0] = BowserMinPos + HealthRatioBowser;
    BowserHealthMove[0,2].v.ob[0] = BowserMinPos + HealthRatioBowser;

Vtx *TPMeterMove = segmented_to_virtual(&TP_Meter_TP_Meter_mesh_vtx_0);
    TPMeterMove[0].v.ob[1] = TPMinPos + TPRatio;
    TPMeterMove[3].v.ob[1] = TPMinPos + TPRatio;
#endif
